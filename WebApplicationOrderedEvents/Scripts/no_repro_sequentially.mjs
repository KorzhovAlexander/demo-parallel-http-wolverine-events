// no_repro_sequentially.mjs
// Node 18+ (Node 20/22/24 ok)
// Run examples:
//   node no_repro_sequentially.mjs
//   RUNS=100 BASE=http://localhost:5198 node no_repro_sequentially.mjs
//   RUNS=200 PARALLEL=5 node no_repro_sequentially.mjs
...
const MARK_READY_ROUTE = "api/v1/mark-ready-controller"; //or mark-ready -> mark-ready-controller
...
async function runOnce(runIndex) {
    ...
    const [m2] = await Promise.all([postJson("/" + MARK_READY_ROUTE, payload, `mark-ready#${runIndex}.2`).catch((e) => ({
        ok: false,
        status: 0,
        statusText: "ERR",
        body: String(e),
